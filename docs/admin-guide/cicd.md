...
The response contains the task ID. Poll `GET /api/project/1/tasks/{task_id}` until the `status` is `success`, `error`, or `stopped`. See [API](/admin-guide/api) for tokens and the built-in Swagger reference.

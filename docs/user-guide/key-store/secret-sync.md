...
Semaphore can connect to an external secret manager — such as **HashiCorp Vault**, **OpenBao**, **AWS Secrets Manager**, **Azure Key Vault**, **Devolutions Server (DVLS)**, or **CyberArk** — and automatically import secrets from it into the Key Store. Instead of copying credentials into Semaphore by hand and keeping them up to date, you point Semaphore at your remote storage and it maintains a local mirror for you.
...
:::tip
Sensible defaults are applied per provider — for example, HashiCorp Vault, OpenBao, and AWS Secrets Manager default to `/` as the separator, Azure Key Vault to `-`, and Devolutions Server to `\` — so in most cases you only need to fill in the path. For CyberArk the path is a Safe name and the separator is not used, because Safes are flat.
:::
...
- Syncing is supported only for **external** storage types (HashiCorp Vault, OpenBao, AWS Secrets Manager, Azure Key Vault, Devolutions Server, CyberArk). The built-in **Database** storage holds secrets natively and has nothing to sync.

...
Die Antwort enthält die Task-ID. Rufen Sie `GET /api/project/1/tasks/{task_id}` ab, bis der `status` `success`, `error` oder `stopped` lautet. Siehe [API](/admin-guide/api) für Tokens und die integrierte Swagger-Referenz.

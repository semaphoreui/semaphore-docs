...
Zamenite čuvare mesta vrednostima iz svoje PingFederate konfiguracije:

* `YOUR_CLIENT_ID` — client ID OIDC aplikacije.
* `YOUR_CLIENT_SECRET` — client secret OIDC aplikacije.
* `YOUR_SEMAPHORE_HOST_AND_PORT` — spoljni host i port Semaphore UI.
* `YOUR_ENVIRONMENT_ID` — identifikator okruženja iz vašeg issuer URL-a.

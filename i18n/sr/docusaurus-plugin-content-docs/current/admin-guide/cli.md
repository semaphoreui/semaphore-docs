...
Većina administratorskih poslova ima posebnu grupu komandi:

| Grupa komandi | Namena |
|---------------|--------|
| [`semaphore users`](/admin-guide/cli/users) | Dodavanje, izmena, uklanjanje i pregled korisnika; upravljanje API tokenima i TOTP-om (2FA). |
| [`semaphore projects`](/admin-guide/cli/projects) | Izvoz i uvoz projekata (Project) u vidu rezervnih kopija. |
| [`semaphore vaults`](/admin-guide/cli/vaults) | Ponovno šifrovanje sačuvanih tajni i pregled upotrebe ključeva za šifrovanje. |
| [`semaphore runner`](/admin-guide/cli/runners) | Rad u runner režimu i registracija/odjava runnera (Runner). |
| [`semaphore migrate`](/admin-guide/cli/migrations) | Primena ili vraćanje migracija baze podataka. |
...
Pokreće Semaphore kao izvršioca zadataka (Task). Pogledajte [Runneri](/admin-guide/cli/runners) za
...
[Migracije baze podataka](/admin-guide/cli/migrations) za primenu ili vraćanje
na određenu verziju.

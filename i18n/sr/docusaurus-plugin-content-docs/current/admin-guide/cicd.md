...
Odgovor sadrži ID zadatka. Proveravajte `GET /api/project/1/tasks/{task_id}` dok `status` ne postane `success`, `error` ili `stopped`. Pogledajte [API](/admin-guide/api) za tokene i ugrađenu Swagger referencu.

...
| 命令组 | 用途 |
|---------------|---------|
| [`semaphore users`](/admin-guide/cli/users) | 添加、修改、删除和查看用户；管理 API 令牌和 TOTP（双因素认证）。 |
| [`semaphore projects`](/admin-guide/cli/projects) | 导出和导入项目（备份）。 |
| [`semaphore vaults`](/admin-guide/cli/vaults) | 重新加密已存储的密钥并查看加密密钥的使用情况。 |
| [`semaphore runner`](/admin-guide/cli/runners) | 以运行器（Runner）模式运行，以及注册/注销运行器。 |
| [`semaphore migrate`](/admin-guide/cli/migrations) | 应用或回滚数据库迁移。 |
...
（`setup`、`register`、`start`、`unregister`）请参阅[运行器](/admin-guide/cli/runners)。
...
请参阅[数据库迁移](/admin-guide/cli/migrations)。

...
- **2 要素認証 (2FA)**<br />TOTP ベースの 2FA を利用でき、すべてのユーザーに推奨されます。ユーザーごとに有効化でき、任意でリカバリーコードを使用できます。設定オプション `auth.totp.enabled` と `auth.totp.allow_recovery` を参照してください。
...
- **コンテナーによる分離**<br />タスクは分離された Docker コンテナー内で実行されます。これらのコンテナーはホストシステムにアクセスできません。

- **最小権限**<br />コンテナーは最小限の権限で実行され、Docker のフラグを使用してさらに制限できます。

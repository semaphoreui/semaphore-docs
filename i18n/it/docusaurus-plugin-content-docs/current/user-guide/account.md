...
| **API Tokens** | Token personali per l'[API REST](/admin-guide/api). |
...
Utilizzare il token nell'intestazione `Authorization: Bearer`, vedere [API](/admin-guide/api). Per revocare un token, eliminarlo dall'elenco.

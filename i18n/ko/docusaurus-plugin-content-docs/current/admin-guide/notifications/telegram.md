...
각 프로젝트는 고유한 채팅 ID를 사용할 수 있습니다. 이를 통해 모든 알림을 같은 채팅으로 보내는 대신 프로젝트별로 알림을 분리할 수 있습니다. 이 설정은 위의 전역 채팅 ID를 재정의합니다. 프로젝트의 [알림](/user-guide/alerts#server-channels) 탭에서 설정하거나, 여러 채팅 또는 포럼 주제를 대상으로 하려면 Telegram 유형의 [프로젝트 알림](/user-guide/alerts#project-alerts)을 만드세요.

...
| <br />`git_attempts`    <hr /> `SEMAPHORE_GIT_ATTEMPTS`<br /><br /> | Quantas vezes um clone ou pull do Git é tentado antes que a tarefa falhe. Usa backoff exponencial (1s, depois 2s, 4s, … até 60s) entre as tentativas. Padrão: `4`. Defina como `1` para desabilitar as repetições. |
...
| <br />`dirs.secrets` <hr /> `SEMAPHORE_SECRETS_PATH`<br /><br /> | Caminho para o diretório onde os segredos são armazenados (por exemplo, arquivos de token do Vault). Padrão: `/tmp/semaphore`. O `secrets_path` legado de nível superior ainda é aceito quando `dirs.secrets` não está definido ou permanece no padrão. |
...
| <br />`schedule.timezone`     <hr /> `SEMAPHORE_SCHEDULE_TIMEZONE` <br /><br /> | Fuso horário usado para agendar tarefas e cron jobs. Padrão: UTC |
...
| <br />`password_login_disable` <hr /> `SEMAPHORE_PASSWORD_LOGIN_DISABLED` <br /><br /> ![Static Badge](https://img.shields.io/badge/v2.10+-red)    <br /><br /> | Nega o login por senha. |
...
| <br />`forwarded_env_vars`     <hr /> `SEMAPHORE_FORWARDED_ENV_VARS` <br /><br /> | Array JSON de variáveis de ambiente do host que serão encaminhadas para as execuções de tarefas. |
| <br />`apps`                   <hr /> `SEMAPHORE_APPS` <br /><br /> | Mapa JSON que contém a configuração dos apps. |
| <br />`use_remote_runner`      <hr /> `SEMAPHORE_USE_REMOTE_RUNNER` <br /><br /> | Habilite para usar um runner remoto. |
| <br />`runner_registration_token` <hr /> `SEMAPHORE_RUNNER_REGISTRATION_TOKEN` <br /><br /> | Token de bootstrap usado pelos runners para se registrarem no servidor. |
| **Assinatura** ||
| <br />`subscription.key` <hr /> `SEMAPHORE_SUBSCRIPTION_KEY` <br /><br /> | Chave ou token da assinatura. Desabilita a ativação pela interface web quando definido. |
| <br />`subscription.key_file` <hr /> `SEMAPHORE_SUBSCRIPTION_KEY_FILE` <br /><br /> | Caminho para o arquivo da chave ou token da assinatura. |
...
| **JWT** ||
| <br />`jwt.enabled` <hr /> `SEMAPHORE_JWT_ENABLED` <br /><br /> | Quando habilitado, o Semaphore emite um JWT de curta duração para cada execução de tarefa e expõe sua chave pública em `/.well-known/jwks.json`. |
| <br />`jwt.issuer` <hr /> `SEMAPHORE_JWT_ISSUER` <br /><br /> | Valor emitido na claim `iss` dos JWTs gerados. |
| <br />`jwt.default_ttl` <hr /> `SEMAPHORE_JWT_DEFAULT_TTL` <br /><br /> | Tempo de vida padrão de um JWT de tarefa emitido, como uma duração do Go (ex.: `30m`, `1h`). Padrão: 1h |
| <br />`jwt.max_ttl` <hr /> `SEMAPHORE_JWT_MAX_TTL` <br /><br /> | Limite superior rígido para o TTL do JWT por template, como uma duração do Go. Padrão: 24h |
| **Runner** ||
...
| <br />`runner.enabled` <hr /> `SEMAPHORE_RUNNER_ENABLED` <br /><br /> | Habilita o runner. |
| <br />`runner.webhook` <hr /> `SEMAPHORE_RUNNER_WEBHOOK` <br /><br /> | URL do webhook para o runner. |
...
| <br />`runner.check_interval_seconds` <hr /> `SEMAPHORE_RUNNER_CHECK_INTERVAL_SECONDS` <br /><br /> | Com que frequência o runner consulta o servidor em busca de novos jobs e reporta o progresso, em segundos. Padrão: 1. Valores maiores reduzem o volume de requisições ao custo de uma captura de jobs ligeiramente mais lenta. |
| <br />`runner.project_id` <hr /> `SEMAPHORE_RUNNER_PROJECT_ID` <br /><br /> | Restringe o runner a um único projeto. |
| <br />`runner.connection.server_ca_cert_file` <hr /> `SEMAPHORE_RUNNER_SERVER_CA_CERT_FILE` <br /><br /> | Bundle PEM usado para verificar o certificado do servidor do Semaphore, além do repositório de confiança do sistema. Defina quando o servidor usar um certificado autoassinado ou de uma CA interna. |
| <br />`runner.connection.skip_tls_verify` <hr /> `SEMAPHORE_RUNNER_SKIP_TLS_VERIFY` <br /><br /> | Desabilita completamente a verificação do certificado do servidor. Inseguro (vulnerável a MITM) — use somente para testes. |
| <br />`runner.executor` <hr /> `SEMAPHORE_RUNNER_EXECUTOR` <br /><br /> | Objeto JSON com a configuração completa do executor do runner (`type` mais as configurações aninhadas `docker` ou `k8s`). Use quando quiser definir todo o bloco do executor a partir de uma única variável de ambiente. |
...
| <br />`runner.executor.k8s.cleanup_grace_seconds` <hr /> `SEMAPHORE_RUNNER_K8S_CLEANUP_GRACE_SECONDS` <br /><br /> | Período de tolerância ao excluir Pods, em segundos. Padrão: 30 |
...
| <br />`runner.executor.docker.tls_verify` <hr /> `SEMAPHORE_RUNNER_DOCKER_TLS_VERIFY` <br /><br /> | Habilita a verificação do certificado TLS para conexões `tcp://`. |
...
| <br />`runner.executor.docker.memory_limit` <hr /> `SEMAPHORE_RUNNER_DOCKER_MEMORY_LIMIT` <br /><br /> | Quando não vazio, limita a memória do contêiner de build (ex.: `2g`). |
...
| <br />`runner.executor.docker.cleanup_grace_seconds` <hr /> `SEMAPHORE_RUNNER_DOCKER_CLEANUP_GRACE_SECONDS` <br /><br /> | Tempo limite passado para `docker stop`, em segundos. Padrão: 30 |
| <br />`runner.executor.docker.privileged` <hr /> `SEMAPHORE_RUNNER_DOCKER_PRIVILEGED` <br /><br /> | Executa o contêiner de build com `--privileged`. Perigoso; desabilitado por padrão. |
...
| **Equipes** ||
| <br />`teams.invites_enabled` <hr /> `SEMAPHORE_TEAMS_INVITES_ENABLED` <br /><br /> | Permite que os usuários convidem membros para as equipes. |
| <br />`teams.invite_type` <hr /> `SEMAPHORE_TEAMS_INVITE_TYPE` <br /><br /> | Tipo de convite: `username` (padrão), `email`, `both`. |
| <br />`teams.members_can_leave` <hr /> `SEMAPHORE_TEAMS_MEMBERS_CAN_LEAVE` <br /><br /> | Permite que os membros saiam das equipes. |
| **Banco de dados** ||
...
| <br />`mysql.user` <hr />`SEMAPHORE_DB_USER`<br /><br /> | Nome do usuário do MySQL.                    |
| <br />`mysql.pass` <hr /> `SEMAPHORE_DB_PASS`<br /><br /> | Senha do usuário do MySQL.              |
...
| <br />`postgres.user` <hr /> `SEMAPHORE_DB_USER`<br /><br /> | Nome do usuário do Postgres.                 |
| <br />`postgres.pass` <hr /> `SEMAPHORE_DB_PASS`<br /><br /> | Senha do usuário do Postgres.           |
...
| <br />`option_encryption` <hr /> `SEMAPHORE_OPTION_ENCRYPTION`<br /><br /> | Chave codificada em Base64 usada para criptografar as opções do banco de dados (a chave de assinatura JWT) com o esquema antigo de chave única (sem rotação). Recorre à chave de acesso quando não definida. |
| <br />`cookie_hash`           <hr /> `SEMAPHORE_COOKIE_HASH`<br /><br /> | Chave HMAC codificada em Base64 usada para assinar os cookies. |
| <br />`cookie_encryption`     <hr /> `SEMAPHORE_COOKIE_ENCRYPTION`<br /><br /> | Chave codificada em Base64 usada para criptografar os cookies. |
| <br />`web_host`       <hr /> `SEMAPHORE_WEB_ROOT`<br /><br /> | Pode ser útil se você quiser usar o Semaphore em um subcaminho, por exemplo: [http://yourdomain.com/semaphore](http://yourdomain.com/semaphore). Não adicione uma `/` no final. |
| <br />`tls.enabled`    <hr /> `SEMAPHORE_TLS_ENABLED`<br /><br /> | Habilita ou desabilita o TLS (HTTPS) para comunicação segura com o servidor do Semaphore. |
...
| <br />`auth.max_session_life_hours` ![Static Badge](https://img.shields.io/badge/v2.20.0-red) <hr /> `SEMAPHORE_AUTH_MAX_SESSION_LIFE_HOURS` ![Static Badge](https://img.shields.io/badge/v2.20.0-red) <br /><br /> | Tempo de vida absoluto de uma sessão de login em horas, contado a partir do login. Quando excedido, o usuário precisa fazer login novamente, mesmo que a sessão tenha estado ativa recentemente. `0` (padrão) significa sem limite absoluto; nesse caso, as sessões expiram somente após 7 dias sem atividade. |
| <br />`mfa.totp.enabled`         <hr /> `SEMAPHORE_TOTP_ENABLED` <br /><br /> | Habilita a autenticação de dois fatores usando TOTP. |
| <br />`mfa.totp.app_name` ![Static Badge](https://img.shields.io/badge/v2.17.0-red) <hr /> `SEMAPHORE_TOTP_ISSUER` ![Static Badge](https://img.shields.io/badge/v2.17.0-red) <br /><br /> | Rótulo do emissor (título do Semaphore) mostrado nos aplicativos autenticadores TOTP. |
...
| <br />`mfa.email.enabled`        <hr /> `SEMAPHORE_EMAIL_2TP_ENABLED` <br /><br /> | Habilita a autenticação multifator baseada em e-mail. |
...
| <br />`mfa.email.allow_create_external_user` <hr /> `SEMAPHORE_EMAIL_2TP_ALLOW_CREATE_EXTERNAL_USER` <br /><br /> | Permite a criação de usuários externos no primeiro login. |
| <br />`mfa.email.allowed_domains` <hr /> `SEMAPHORE_EMAIL_2TP_ALLOWED_DOMAINS` <br /><br /> | Array JSON de domínios de e-mail permitidos. |
| <br />`mfa.email.disable_for_oidc` <hr /> `SEMAPHORE_EMAIL_2TP_DISABLE_FOR_OIDC` <br /><br /> | Desabilita o MFA por e-mail para usuários autenticados via OIDC. |
| **Criptografia** ||
...
| <br />`encryption.keys_poll_interval` <hr /> `SEMAPHORE_ENCRYPTION_KEYS_POLL_INTERVAL` <br /><br /> | Com que frequência o `keys_file` é verificado quanto a alterações (uma duração do Go, como `15s`). `0` desabilita a verificação (um SIGHUP ainda força a recarga). Padrão: 15s |
| **Processo** ||
...
| <br />`process.chroot`        <hr /> `SEMAPHORE_PROCESS_CHROOT` <br /><br /> | Diretório de chroot para os processos encapsulados. |
...
| <br />`process.app_namespaces.user`  <hr /> `SEMAPHORE_PROCESS_APP_NS_USER` <br /><br /> | Isola UIDs/GIDs (`CLONE_NEWUSER`) para as execuções de apps. Somente Linux. |
| <br />`process.app_namespaces.mount` <hr /> `SEMAPHORE_PROCESS_APP_NS_MOUNT` <br /><br /> | Oculta os pontos de montagem do host, como o tmpfs de segredos (`CLONE_NEWNS`), das execuções de apps. Somente Linux. |
| <br />`process.app_namespaces.pid`   <hr /> `SEMAPHORE_PROCESS_APP_NS_PID` <br /><br /> | Oculta os processos do host das execuções de apps (`CLONE_NEWPID`). Somente Linux. |
| <br />`process.app_namespaces.ipc`   <hr /> `SEMAPHORE_PROCESS_APP_NS_IPC` <br /><br /> | Isola o IPC SysV e as filas de mensagens POSIX (`CLONE_NEWIPC`) para as execuções de apps. Somente Linux. |
| <br />`process.app_namespaces.uts`   <hr /> `SEMAPHORE_PROCESS_APP_NS_UTS` <br /><br /> | Isola o hostname e o domínio (`CLONE_NEWUTS`) para as execuções de apps. Somente Linux. |
| **E-mail** ||
...
| <br />`email_secure`   <hr /> `SEMAPHORE_EMAIL_SECURE`<br /><br /> | Habilita o StartTLS para converter uma conexão SMTP não criptografada em uma conexão segura e criptografada. |
...
| <br />`email_alert`    <hr /> `SEMAPHORE_EMAIL_ALERT`<br /><br /> | Flag que habilita os alertas por e-mail. |
| **Mensageiros** ||
| <br />`telegram_alert` <hr /> `SEMAPHORE_TELEGRAM_ALERT`<br /><br /> | Defina como True para habilitar o envio de alertas para o Telegram. Deve ser usado em combinação com `telegram_chat` e `telegram_token`. |
...
| <br />`slack_alert`    <hr /> `SEMAPHORE_SLACK_ALERT`<br /><br /> | Defina como True para habilitar o envio de alertas para o Slack. Deve ser usado em combinação com `slack_url`                          |
...
| <br />`microsoft_teams_alert` <hr /> `SEMAPHORE_MICROSOFT_TEAMS_ALERT` <br /><br /> | Flag que habilita os alertas do Microsoft Teams. |
| <br />`microsoft_teams_url`   <hr /> `SEMAPHORE_MICROSOFT_TEAMS_URL` <br /><br /> | URL do webhook do Microsoft Teams. |
| <br />`rocketchat_alert`      <hr /> `SEMAPHORE_ROCKETCHAT_ALERT` <br /><br /> | Defina como True para habilitar o envio de alertas para o Rocket.Chat. Deve ser usado em combinação com `rocketchat_url`. Disponível desde a v2.9.56.  |
...
| <br />`dingtalk_alert`        <hr /> `SEMAPHORE_DINGTALK_ALERT` <br /><br /> | Habilita os alertas do DingTalk. |
| <br />`dingtalk_url`          <hr /> `SEMAPHORE_DINGTALK_URL` <br /><br /> | URL do webhook do mensageiro DingTalk. |
| <br />`gotify_alert`          <hr /> `SEMAPHORE_GOTIFY_ALERT` <br /><br /> | Habilita os alertas do Gotify. |
...
| **LDAP** ||
| <br />`ldap_enable`           <hr /> `SEMAPHORE_LDAP_ENABLE` <br /><br /> | Flag que habilita a autenticação LDAP. |
| <br />`ldap_needtls`          <hr /> `SEMAPHORE_LDAP_NEEDTLS` <br /><br /> | Flag para habilitar ou desabilitar o TLS nas conexões LDAP. |
...
| <br />`ldap_server`           <hr /> `SEMAPHORE_LDAP_SERVER` <br /><br /> | O hostname e a porta do servidor LDAP (ex.: ldap-server.com:1389). |
| <br />`ldap_searchdn`         <hr /> `SEMAPHORE_LDAP_SEARCH_DN` <br /><br /> | O nome distinto (DN) base usado para pesquisar usuários no diretório LDAP (ex.: dc=example,dc=org). |
| <br />`ldap_searchfilter`     <hr /> `SEMAPHORE_LDAP_SEARCH_FILTER` <br /><br /> | O filtro usado para pesquisar usuários no diretório LDAP (ex.: (&(objectClass=inetOrgPerson)(uid=%s))). |
...
| **Logs** ||
| <br />`log.events.format`      <Pro /> <hr /> `SEMAPHORE_EVENT_LOG_FORMAT` <br /><br /> | Formato do log de eventos. Pode ser `json` ou vazio para texto. |
| <br />`log.events.enabled`     <Pro /> <hr /> `SEMAPHORE_EVENT_LOG_ENABLED` <br /><br /> | Habilita ou desabilita o log de eventos. |
| <br />`log.events.logger`      <Pro /> <hr /> `SEMAPHORE_EVENT_LOGGER` <br /><br /> | Mapa JSON que contém a configuração do logger de eventos. |
| <br />`log.tasks.format`       <Pro /> <hr /> `SEMAPHORE_TASK_LOG_FORMAT` <br /><br /> | Formato do log de tarefas. Pode ser `json` ou vazio para texto. |
| <br />`log.tasks.enabled`      <Pro /> <hr /> `SEMAPHORE_TASK_LOG_ENABLED` <br /><br /> | Habilita ou desabilita o log de tarefas. |
| <br />`log.tasks.logger`       <Pro /> <hr /> `SEMAPHORE_TASK_LOGGER` <br /><br /> | Mapa JSON que contém a configuração do logger de tarefas. |
| <br />`log.tasks.result_logger`  <Pro /> <hr /> `SEMAPHORE_TASK_RESULT_LOGGER` <br /><br /> | Mapa JSON que contém a configuração do logger de resultados das tarefas. |
| <br />`syslog.enabled` <Pro /> <hr /> `SEMAPHORE_SYSLOG_ENABLED` <br /><br /> | Habilita ou desabilita a gravação de logs no servidor syslog configurado. |
| <br />`syslog.network` <Pro /> <hr /> `SEMAPHORE_SYSLOG_NETWORK` <br /><br /> | Protocolo usado para se conectar ao servidor Syslog: `udp` ou `tcp`. |
| <br />`syslog.address` <Pro /> <hr /> `SEMAPHORE_SYSLOG_ADDRESS` <br /><br /> | Hostname e porta do servidor Syslog. Exemplo: `localhost:514`. |
| <br />`syslog.tag` <Pro /> <hr /> `SEMAPHORE_SYSLOG_TAG` <br /><br /> | A tag usada para marcar os registros do Semaphore UI no servidor Syslog. |
| <br />`syslog.format` <Pro /> <hr /> `SEMAPHORE_SYSLOG_FORMAT` <br /><br /> | Formato das mensagens do Syslog. Pode ser `rfc5424` ou vazio para o padrão. |
| **Depuração** ||
...
| <br />`ha.enabled` <Enterprise /> <hr /> `SEMAPHORE_HA_ENABLED` <br /><br /> | Habilita o modo de alta disponibilidade (HA). |
| <br />`ha.node_id` <Enterprise /><hr /> `SEMAPHORE_HA_NODE_ID` <br /><br /> | Identificador único do nó HA. |
| <br />`ha.redis.addr` <Enterprise /> <hr /> `SEMAPHORE_HA_REDIS_ADDR` <br /><br /> | Endereço do servidor Redis usado para HA. Exemplo: `localhost:6379`. |
| <br />`ha.redis.db` <Enterprise /><hr /> `SEMAPHORE_HA_REDIS_DB` <br /><br /> | Número do banco de dados Redis. |
| <br />`ha.redis.pass` <Enterprise /><hr /> `SEMAPHORE_HA_REDIS_PASS` <br /><br /> | Senha do servidor Redis. |
| <br />`ha.redis.user` <Enterprise /><hr /> `SEMAPHORE_HA_REDIS_USER` <br /><br /> | Nome de usuário do servidor Redis. |
| <br />`ha.redis.tls` <Enterprise /><hr /> `SEMAPHORE_HA_REDIS_TLS` <br /><br /> | Habilita o TLS para a conexão com o Redis. |
| <br />`ha.redis.tls_skip_verify` <Enterprise /><hr /> `SEMAPHORE_HA_REDIS_TLS_SKIP_VERIFY` <br /><br /> | Ignora a verificação do certificado TLS na conexão com o Redis. |

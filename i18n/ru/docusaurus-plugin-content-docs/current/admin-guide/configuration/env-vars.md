...
Semaphore передаёт процессам, которые запускает, не всё своё окружение. В частности, переменные прокси попадают в задачу или в клонирование `git` только если они перечислены в `forwarded_env_vars` или заданы в `env_vars`.
